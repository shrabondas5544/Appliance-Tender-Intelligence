"""
Engine 2: Dhaka Tribune e-Paper OCR Scraper.

Dynamically discovers and scans all pages of the daily edition, performs Tesseract OCR (English),
...
class DhakaTribuneScraper(BaseScraper):
    ...
    def get_page_image_urls(self, target_date: date) -> List[dict]:
        """Dynamically discovers all page image URLs for target_date by probing sequentially."""
        ...
        try:
            while True:
                # Format: https://epaper-media.dhakatribune.com/en/epaper/YYYY-MM-DD/main-edition/{page}/full.jpg
                url = f"https://epaper-media.dhakatribune.com/en/epaper/{date_str}/main-edition/{page_num}/full.jpg"
                
                # Probing page existence using HTTP HEAD
                resp = requests.head(url, headers=HEADERS, timeout=10)
                if resp.status_code != 200:
                    break  # Stop when we hit a page that doesn't exist (e.g. page 13)
            ...
            logger.error("[%s] Failed to discover pages dynamically: %s", self.source_portal, e)
    ...
    def scrape_date(self, target_date: date) -> List[TenderRecord]:
        ...
        for idx, page in enumerate(pages, start=1):
            ...
            image_url = page["url"]
            filename = f"dt_{target_date.strftime('%Y-%m-%d')}_p{page_num:02d}.jpg"
            ...
            # 2. Extract text via OCR Engine (English)
            ...
            tender_id = f"DT-{target_date.strftime('%Y%m%d')}-P{page_num:02d}-{category}"
            title = f"Appliance Procurement Notice on Page {page_num}"

            # Try to identify procuring entity using organization keywords
            procuring_entity = "Classified Notice"
            m_org = re.search(r"([A-Za-z0-9\s,\.]{3,40}\s*(?:Bank|Company|Limited|Ltd|Corporation|Authority|University|Hospital))", text, re.IGNORECASE)
            if m_org:
